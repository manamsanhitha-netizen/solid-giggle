...
st.markdown("""
    ...
    /* Compact Sidebar Navigation Buttons */
    ...
    h1, h2, h3, h4 {margin-top: 2px; margin-bottom: 5px;}
    </style>
...
# --- LOGIN SCREEN ---
def login_page():
    st.markdown("<h2 style='text-align: center; color: #4A90E2;'>🎓 FinSmart Academy</h2>", unsafe_allow_html=True)
    st.markdown("<p style='text-align: center; color: gray; margin-top:-10px; font-size:14px;'>Simple Financial Tools for Students</p>", unsafe_allow_html=True)
...
def show_finance_desk(profile, username):
    ...
    with left_panel:
        ...
        <div class="metric-card">
            <table style="width:100%; border:none; margin:0;">
                <tr>
                    <td style="padding:4px; font-size:14px; color:#6b7280;">Profile Type:</td>
                    <td style="padding:4px; font-size:14px; font-weight:600; text-align:right;">{standing}</td>
                </tr>
                <tr>
                    <td style="padding:4px; font-size:14px; color:#6b7280;">Learning Points:</td>
                    <td style="padding:4px; font-size:14px; font-weight:600; text-align:right; color:#4A90E2;">🏅 {points} XP</td>
                </tr>
                <tr>
                    <td style="padding:4px; font-size:14px; color:#6b7280;">Academy Rank:</td>
    ...
    with right_panel:
        ...
        if res.data:
            ...
            st.markdown("<div style='height:160px; display:flex; align-items:center; justify-content:center; background:#f3f4f6; border-radius:8px; color:gray; font-size:13px;'>No expenses logged yet.</div>", unsafe_allow_html=True)
...
def show_micro_courses(profile, username):
    ...
    with h_col1: st.markdown("### 📚 Bite-Sized Finance Courses")
    with h_col2:
        sel_course = st.selectbox("Select Course Topic", courses, index=st.session_state.course_idx, label_visibility="collapsed")
    ...
    st.progress(current_step / total_steps, text=f"Total Progress: {int((current_step/total_steps)*100)}% Complete (Page {current_step}/{total_steps})")
    ...
    if c_idx == 0:
        if p_idx == 1:
            ...
            st.write("Want to know how fast your money will double? Divide 72 by your expected yearly interest rate.")
            st.latex(r"Years\ to\ Double = \frac{72}{Interest\ Rate}")
        elif p_idx == 3:
            st.markdown("#### Course 1: The Formula")
            st.write("The simple exponential formula that rules the investment world:")
            ...
            if st.button("Submit Quiz Answer"):
                ...
                else: st.error("Incorrect. Remember, the 10% interest in year 2 is calculated on ₹1,100, not your original ₹1,000.")

    elif c_idx == 1:
        if p_idx == 1:
            st.markdown("#### Course 2: The 50/30/20 Budget Rule")
            st.write("Budgeting fails when it is too strict. The 50/30/20 rule is a simple guide to split your take-home pay check cleanly into three clear categories without feeling restricted.")
        elif p_idx == 2:
        ...
        if p_idx < 4:
            ...
            ans = st.radio("What is the safest, most recommended path to build personal wealth?", ["Keep all savings in cash at home", "Pay off expensive debts first, save an emergency fund, and invest in diversified index funds", "Buy high-risk single stocks with short-term money"])
            if st.button("Submit Quiz Answer"):
                if "Pay off expensive debts" in ans: st.success("Correct! +25 XP Added."); supabase.table("profiles").update({"points": current_points + 25}).eq("username", username).execute(); st.rerun()
...
def show_budgeting(username):
    ...
    with col2:
        ...
        st.markdown(f"""
        <div class="metric-card">
            <span style="font-size:12px; color:gray;">Monthly Budget Usage</span><br/>
            <span style="font-size:20px; font-weight:700; color:#4A90E2;">₹{total:.2f} Used / ₹25,000 Target</span>
        </div>
        """, unsafe_allow_html=True)
...
def show_savings():
    ...
    with col2:
        ...
        st.markdown(f"""
        <div class="metric-card">
            <span style="font-size:11px; color:gray; font-weight:bold;">SAVINGS GROWTH COMPARISON</span>
            <hr style="margin:6px 0; border-color:#eee;"/>
            <div style="display:flex; justify-content:between; font-size:13px; margin:4px 0;">
                <span>Regular Savings Account (~3.5% Return):</span>
            ...
            <div style="display:flex; justify-content:between; font-size:13px; margin:4px 0; color:#10b981;">
                <span>Fixed Deposit (FD) (~7.1% Return):</span>
                <span style="margin-left:auto; font-weight:600;">₹{fd_final:.2f}</span>
            </div>
            <hr style="margin:6px 0; border-color:#eee;"/>
            <span style="font-size:11px; color:#4A90E2; font-weight:700;">💡 Tip: By choosing an FD, you make an extra ₹{difference:.2f} over a normal savings account!</span>
        </div>
        """, unsafe_allow_html=True)
...
def show_splitting():
    ...
    with col1:
        ...
        people = st.slider("Split Between (Number of People)", 2, 12, 3)
    ...
    with col2:
        ...
        <div class="metric-card" style="text-align:center; padding:18px 16px; border-left: 6px solid #10b981; margin-bottom: 8px;">
            <span style="font-size:13px; color:gray;">EACH PERSON OWES:</span><br/>
        ...
        if vpa:
            ...
            st.caption("📱 Copy this link format to send a quick payment request to your friends.")
...
if st.session_state.username is None:
    ...
    with st.sidebar:
        ...
        st.markdown(f"🏆 Total Points: **{user_points} XP**")
        st.markdown("<hr style='margin: 8px 0;'/>", unsafe_allow_html=True)
        
        # Cleaner button-driven menu navigation
    ...
    # Route navigation pathways cleanly
